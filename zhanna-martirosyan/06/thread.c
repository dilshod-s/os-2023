...
void* worker(void* data) {
    ...
    int blockSize = N / countThreads; //размер блока для каждого потока вычисляет
...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        printf("Аргумента должно быть 2: название файла и количество потоков.\n");
        return 1;
    }

    countThreads = atoi(argv[1]); //инициализируем количество потоков (atoi - из строки в целое число)
    ...
    localResult = (uint64_t*)malloc(countThreads * sizeof(uint64_t)); //массив для локальных результатов размером в количество потоков

    //запускаем потоки
    pthread_t threads[countThreads]; //тут находятся ID потоков
    int threadInd[countThreads]; //тут индексы потоков(от 0 до (numThreads - 1))
    ...
    for (int i = 0; i < countThreads; ++i) { //создаем и запускаем потоки, передавая каждому индекс потока
    ...
    for (int i = 0; i < countThreads; ++i) { //завершаем потоки
    ...
    uint64_t result = 0;
    for (int i = 0; i < countThreads; ++i) { //вычисляем xor из локальных результатов(общую побитовую сумму)
    ...
    //умножаем на 1000000, чтоб преобразовать секунды в микросекунды
