...
{
    memory->data[0] = 1;
    for (size_t i = 1; i < MEMSIZE / PGSIZE; ++i)
        memory->data[i] = 0;
}
...
int main()
{
    ...
    std::cout << "Memory:\t\t\t" << static_cast<void*>(memory.data) << std::endl;

    size_t size = 512;
    void* p1 = alloc(&memory, size);
    std::cout << "size: " << size << "\tp1:\t" << p1 << std::endl;

    size = 1030;
    void* p2 = alloc(&memory, size);
    std::cout << "size: " << size << "\tp2:\t" << p2 << std::endl;
